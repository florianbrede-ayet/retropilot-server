...
function initializeStorage() {
  ...
  if (verifiedPath != null) logger.info(`Verified storage path ${verifiedPath}`);
  else {
...
function moveUploadedFile(buffer, directory, filename) {
  ...
  if (config.storagePath.lastIndexOf('/') !== config.storagePath.length - 1) directory = `/${directory}`;
...
function processSegmentVideo(qcameraPath) {
  ...
  return new Promise((resolve, reject) => {
    ffprobe(qcameraPath, { path: ffprobeStatic.path })
      .then((info) => {
        if (info.streams !== undefined && info.streams[0] !== undefined && info.streams[0].duration !== undefined) qcamera_duration = info.streams[0].duration;
...
function processSegmentsRecursive() {
  if (segmentProcessQueue.length <= segmentProcessPosition) return updateDrives();
  ...
  Promise.all([p1, p2]).then((values) => {
    (async () => {
      ...
      if (rlog_CarParams != null) affectedDriveCarParams[driveIdentifier] = rlog_CarParams;
      if (rlog_InitData != null) affectedDriveInitData[driveIdentifier] = rlog_InitData;

      segmentProcessPosition++;
      setTimeout(() => { processSegmentsRecursive(); }, 0);
    })();
  }).catch((error) => {
    logger.error(error);
  });
...
async function updateSegments() {
  ...
  if (segmentProcessQueue.length > 0) processSegmentsRecursive();
